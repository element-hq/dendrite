...
func UpNormalizeServerNames(ctx context.Context, tx *sql.Tx) error {
	...
	for _, query := range checks {
		switch err := tx.QueryRowContext(ctx, query).Scan(&canonical, &count); err {
		case sql.ErrNoRows:
		case nil:
...
`
	...
	statements := []string{
		...
		`UPDATE keydb_server_keys SET server_name = LOWER(server_name), server_name_and_key_id = LOWER(server_name) || char(31) || server_key_id WHERE server_name <> LOWER(server_name)`,
	}
