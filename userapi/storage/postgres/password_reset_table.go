...
func NewPostgresPasswordResetTokensTable(db *sql.DB) (tables.PasswordResetTokensTable, error) {
	s := &passwordResetStatements{}
	if _, err := db.Exec(passwordResetTokensSchema); err != nil {
		return nil, fmt.Errorf("failed to ensure password reset token schema: %w", err)
	}
