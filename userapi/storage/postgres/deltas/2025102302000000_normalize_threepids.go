...
const duplicateThreePIDCheckSQL = `
...
func UpNormalizeThreePIDs(ctx context.Context, tx *sql.Tx) error {
	row := tx.QueryRowContext(ctx, duplicateThreePIDCheckSQL)
	var addr string
	var medium string
	var count int
	...
	if _, err := tx.ExecContext(ctx, normalizeThreePIDSQL); err != nil {
		return fmt.Errorf("normalize threepids: failed to canonicalise data: %w", err)
	}

	if _, err := tx.ExecContext(ctx, createLowerThreePIDIndexSQL); err != nil {
