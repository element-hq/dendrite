...
UPDATE mediaapi_thumbnail AS t
SET quarantined = TRUE,
    quarantined_at = $2,
    quarantined_by = $3,
    quarantine_reason = $4
FROM mediaapi_media_repository AS m
WHERE m.user_id = $1 AND m.media_id = t.media_id AND m.media_origin = t.media_origin AND t.quarantined = FALSE
`
